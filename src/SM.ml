...
| instruction :: rest_program ->
   match instruction with
      ...
      | CALL (f_name, _, _) -> eval env ((rest_program, st)::cstack, stack, c) (env#labeled f_name)
...
let rec compile_expr expr =
    ...
    | Expr.Call (f_name, args) -> concat (map compile_expr args) @ [CALL (f_name, length args, false)]
...
let rec compile_statements stmts =
    ...
    | Stmt.Call (f_name, args) -> concat (map compile_expr args) @ [CALL (f_name, length args, true)]
...
let rec compile (defs, program) =
  (compile_statements program) @ [END] @ (concat (map compile_definition defs))